...
#+begin_src nim
  # Nice addional effect - you can now comment on different flags and
  # use checks/loops without worrying about correct
  # spacing/concatnation/prefixes etc.
  let dockerCmd = makeGnuCmd("docker").withIt do:
    it.subCmd "run" # Add subcommand
    it - "i"
    it - "i"
    if doCleanup:
...
- ~shExec~ - execute shell command, redirect output into parent
  streams.
