...
proc `[]=`*[A, B](t: var Map[A, B]; key: A; val: B) =
  ...
  ## * `hasKeyOrPut proc<#hasKeyOrPut,Map[A,B],A,B>`_
  ## * `mgetOrPut proc<#mgetOrPut,Map[A,B],A,B>`_
  ## * `del proc<#del,Map[A,B],A>`_ for removing a key from the table
  runnableExamples:
...
proc `[]=`*[A, B](t: MapRef[A, B], key: A, val: B) =
  ...
  ## * `del proc<#del,MapRef[A,B],A>`_ for removing a key from the table
  runnableExamples:
...
proc del*[A, B](t: MapRef[A, B], key: A) =
  ...
  ## * `clear proc<#clear,MapRef[A,B]>`_ to empty the whole table
  runnableExamples:
...
proc `[]=`*[A, B](t: var OrderedMap[A, B]; k: A; v: B) =
  ...
  ## * `del proc<#del,OrderedMap[A,B],A>`_ for removing a key from the table
  runnableExamples:
...
proc del*[A, B](t: var OrderedMap[A, B], key: A) =
  ...
  ## * `clear proc<#clear,OrderedMap[A,B]>`_ to empty the whole table
  runnableExamples:
...
proc `[]=`*[A, B](t: OrderedMapRef[A, B], key: A, val: B) =
  ...
  ## * `del proc<#del,OrderedMapRef[A,B],A>`_ for removing a key from the table
  runnableExamples:
...
proc del*[A, B](t: OrderedMapRef[A, B], key: A) =
  ...
  ## * `clear proc<#clear,OrderedMapRef[A,B]>`_ to empty the whole table
  runnableExamples:
...
proc del*[A](t: var CountMap[A], key: A) =
  ...
  ## * `clear proc<#clear,CountMap[A]>`_ to empty the whole table
  runnableExamples:
...
proc del*[A](t: CountMapRef[A], key: A) =
  ...
  ## * `clear proc<#clear,CountMapRef[A]>`_ to empty the whole table
  del(t[], key)
...
iterator mvaluesFrom*[A, B](b: Map[A, B], fromKey: A): var B =
  ## Iterates over the values in the table from the given key to the end.
  for e in entriesFrom(b, fromKey):
    yield e.mval

iterator mvaluesBetween*[A, B](
    b: Map[A, B], fromKey: A, toKey: A): B =
...
iterator mpairsFrom*[A, B](
    b: Map[A, B], fromKey: A): tuple[key: A, val: var B] =
...
iterator mpairsBetween*[A, B](
    b: Map[A, B], fromKey: A, toKey: A): tuple[key: A, val: var B] =
...
proc firstPair*[A, B](table: Map[A, B]): tuple[key: A, val: B] =
...
proc lastPair*[A, B](table: Map[A, B]): tuple[key: A, val: B] =
  var node {.cursor.} = table.root
  assert node.m > 0
  # pprint node
  while true:
...
proc searchOrBefore[A, B](b: Map[A, B], key: A): Cursor[A, B] =
  ## Calculates the cursor pointing to the given key.
  var a = b.root
  var exact = false
  while not a.isNil:
...
iterator entriesBefore[A, B](
    b: Map[A, B], fromKey: A, withKey: bool = false): CursorPosition[A, B] =
  # Iterates the sorted table from the given key to the end.
...
iterator valuesBefore*[A, B](
    b: Map[A, B], fromKey: A, withKey: bool = false): B =
...
iterator pairsBefore*[A, B](
    b: Map[A, B], fromKey: A, withKey: bool = false): tuple[key: B, val: B] =
  ## Iterates over the values in the table from the given key to the end.
  for e in entriesBefore(b, fromKey, withKey):
    yield (e.key, e.val)
